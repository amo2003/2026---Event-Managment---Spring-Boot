...
const IncidentListPage = () => {
  ...
  const openChatDock = (incident) => {
    if (!isAssignedToLoggedInOfficer(incident)) return;

    setOpenChats((current) => {
      const alreadyOpen = current.some((item) => item.id === incident.id);
      if (alreadyOpen) return current;

      const next = [...current, incident];
  ...
  const closeChatDock = (incidentId) => {
    setOpenChats((current) => current.filter((item) => item.id !== incidentId));
  };
  ...
  const renderActionCell = (item) => {
    ...
    if (!isAssignedToLoggedInOfficer(item)) {
      return <span className="muted-note">Assigned to {item.assignedOfficerName}</span>;
    }
    ...
    return <span className="muted-note">No action</span>;
  };

  const renderChatCell = (item) => {
    if (!item.assignedOfficerName) {
      return <span className="muted-note">No officer</span>;
    }

    if (!isAssignedToLoggedInOfficer(item)) {
      return <span className="muted-note">Not assigned</span>;
    }

    return (
      <button
        type="button"
        className="mini-btn chat-table-button"
        onClick={() => openChatDock(item)}
      >
        Chat
      </button>
    );
    ...
    <div className="page-stack">
              ...
              {incidents.length === 0 ? (
                ...
                incidents.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{renderChatCell(item)}</td>
                    <td>
                      <Link to={`/rincidents/${item.id}`} className="mini-btn dark">
                        Open
                      </Link>
                    </td>
                  </tr>
      ...
      {openChats.length > 0 && (
        <div className="chat-dock">
          {openChats.map((chatIncident) => (
            <div key={chatIncident.id} className="dock-chat-window">
              <button
                type="button"
                className="dock-chat-close"
                onClick={() => closeChatDock(chatIncident.id)}
              >
                ×
              </button>
              ...
              <IncidentChatBox
                ...
                title={`Chat #${chatIncident.id}`}
                subtitle={`${chatIncident.reportedBy} • ${chatIncident.incidentType}`}
                compact
              />
            </div>
          ))}
